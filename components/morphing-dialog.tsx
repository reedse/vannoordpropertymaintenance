  ...
  useEffect(() => {
    if (isOpen) {
      ...
      const timer = setTimeout(() => {
        const dialogContent = document.querySelector(`[id^="dialog-content-"]`)
        if (dialogContent) {
...
function MorphingDialogImage({ src, alt, className, style, priority, sizes, webpSrc }: MorphingDialogImageProps) {
  const { uniqueId } = useMorphingDialog()

  return (
    <motion.div
      layoutId={`dialog-image-container-${uniqueId}`}
      className={cn("overflow-hidden", className)}
      style={style}
    >
      <picture>
        <source srcSet={webpSrc || src.replace(/\.(jpg|jpeg|png)$/i, '.webp')} type="image/webp" />
        <img 
          src={src} 
          alt={alt} 
